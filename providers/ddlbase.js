 ...
 * Generated: 2026-06-02T18:56:08.807Z
 */
        ...
        for (const download of data.downloads) {
          ...
          if (Array.isArray(download.sources)) {
            for (const source of download.sources) {
              const displayLabel = `${download.title}${sizeInfo}
Source: ${source.name}`;
              streamsResult.push({
                name: displayLabel,
                // Controls both lines on the UI card
                title: download.title,
                // Hidden fallback asset
                url: source.url,
                // Playable media address
                quality: extractedQuality
                // Resolution badge (1080p, 2160p, etc.)
              });
            }
          }
        }
      ...
      console.error("[DDL Base] Layout reformatting error:", error.message);
    }
