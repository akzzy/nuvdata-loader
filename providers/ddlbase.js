 ...
 * Generated: 2026-06-02T18:45:21.959Z
 */
...
function getStreams(tmdbId, mediaType) {
  return __async(this, null, function* () {
    ...
    try {
        ...
        for (const download of data.downloads) {
          ...
          if (Array.isArray(download.sources)) {
            for (const source of download.sources) {
              const cleanPrefix = `[DDL Base | ${source.name}] `;
              streamsResult.push({
                name: `DDL Base \u{1F4E5} ${source.name}`,
                // Fallback for legacy builds
                title: `${cleanPrefix}${download.title}${sizeInfo}`,
                // Native layout target
                url: source.url,
      ...
      console.error("[DDL Base] Overhaul-safe direct link mapping error:", error.message);
    }
