 ...
 * Generated: 2026-01-14T16:56:48.062Z
 */
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      const cardsStart = searchHtml.indexOf('class="post-cards"');
      if (cardsStart === -1) {
        console.log("[MoviesMod] No 'post-cards' container found. No results.");
        return [];
      }
      const resultsChunk = searchHtml.substring(cardsStart);
      const articleRegex = /<article[^>]*class="[^"]*latestPost[^"]*"[\s\S]*?<a\s+href="([^"]+)"\s+title="([^"]+)"/g;
      let match;
      let bestPostUrl = null;
      let bestScore = 0;
      while ((match = articleRegex.exec(resultsChunk)) !== null) {
        const url = match[1];
        const title = match[2];
        ...
        console.log(`[MoviesMod] Found Candidate: ${title} (Score: ${score})`);
        if (score > bestScore) {
          bestScore = score;
          bestPostUrl = url;
        }
      ...
      if (!bestPostUrl) {
        console.log(`[MoviesMod] No valid post found for: ${titleQuery}`);
        return [];
      }
      console.log(`[MoviesMod] Selected Post: ${bestPostUrl}`);
