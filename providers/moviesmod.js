 ...
 * Generated: 2026-01-14T16:21:52.836Z
 */
...
function resolveSidLink(sidUrl) {
  return __async(this, null, function* () {
    try {
      console.log(`[MoviesMod] Bypassing SID: ${sidUrl}`);
      ...
      console.log(`[MoviesMod] SID Error: ${e.message}`);
      return null;
    }
  });
}
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      while ((match = postRegex.exec(searchHtml)) !== null) {
        const url = match[1];
        const title = match[2];
        if (title.toLowerCase().includes(mediaType === "movie" ? meta.title.toLowerCase() : meta.name.toLowerCase())) {
          if (mediaType === "tv") {
            if (season && title.includes(`Season ${season}`)) {
              postUrl = url;
              break;
            }
            if (!postUrl)
            ...
            if (title.includes((_b = meta.release_date) == null ? void 0 : _b.substring(0, 4))) {
              postUrl = url;
              break;
            }
          }
      ...
      if (!postUrl) {
        console.log("[MoviesMod] No matching post found.");
        return [];
      }
      console.log(`[MoviesMod] Found Post: ${postUrl}`);
      const postRes = yield makeRequest(postUrl);
      ...
      while ((linkMatch = linkRegex.exec(postHtml)) !== null) {
        ...
        if (linkText.includes("4k") || linkText.includes("2160p"))
          ...
          continue;
        if (mediaType === "tv") {
          if (!linkText.includes(`Episode ${episode}`) && !linkText.includes("Zip")) {
          }
        }
        if (streams.length >= 5)
          break;
