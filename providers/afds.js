 ...
 * Generated: 2026-04-09T19:45:56.344Z
 */
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (!searchRes.ok) {
        console.log(`[AFDS] Search failed with status: ${searchRes.status}. Token might be expired.`);
        return [];
      }
      const searchData = yield searchRes.json();
      if (!searchData || !searchData.files || searchData.files.length === 0) {
        console.log("[AFDS] Search returned empty.");
        return [];
      }
      const candidates = [];
