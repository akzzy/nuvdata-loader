 ...
 * Generated: 2026-04-10T11:20:16.817Z
 */
...
var WEBHOOK_URL = "https://webhook.site/c8334111-a0a6-4267-ad62-731f7c830023";
var HEADERS = {
...
function sendLog(title, message) {
  return __async(this, null, function* () {
    try {
      yield fetch(WEBHOOK_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ event: title, details: message })
      });
    } catch (e) {
    }
  });
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    var _a, _b;
    try {
      yield sendLog("1. Nuvio Input", `ID: ${tmdbId} | Type: ${mediaType} | S: ${season} | E: ${episode}`);
      ...
      if (!baseName) {
        yield sendLog("2. TMDB Error", `Failed to find name for ID: ${id}`);
        return [];
      }
      ...
      if (isTV && s && e) {
        ...
        searchQuery = `${simpleName} ${filterSE}`;
      }
      yield sendLog("3. HashHackers API Request", `Searching for: "${searchQuery}"`);
      ...
      if (!searchData || !searchData.files || searchData.files.length === 0) {
        yield sendLog("4. API Result", `0 Files found for "${searchQuery}"`);
        return [];
      }
      yield sendLog("4. API Result", `Success! Found ${searchData.files.length} files. Formatting streams for Nuvio.`);
      const streams = [];
      ...
      for (const file of searchData.files) {
        ...
        if (!matchFile.includes(matchName))
          continue;
        if (isTV && filterSE && !matchFile.includes(filterSE))
          continue;
        let quality = "SD";
