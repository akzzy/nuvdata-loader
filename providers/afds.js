 ...
 * Generated: 2026-04-09T19:52:07.238Z
 */
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      const year = mediaType === "movie" && meta.release_date ? meta.release_date.substring(0, 4) : null;
      console.log(`[AFDS] Searching TMDB Title: "${titleQuery}", Year: ${year}`);
      const searchUrl = `${API_BASE}/mix_media_files/search?q=${encodeURIComponent(titleQuery)}&page=1`;
      console.log(`[AFDS] Fetching: ${searchUrl}`);
      const searchRes = yield fetch(searchUrl, { headers: HEADERS });
      if (!searchRes.ok) {
        console.log(`[AFDS] API Error ${searchRes.status}. Check if Token is expired!`);
        return [];
      }
      const searchData = yield searchRes.json();
      if (!searchData || !searchData.files || searchData.files.length === 0) {
        console.log("[AFDS] API returned 0 files.");
        return [];
      }
      console.log(`[AFDS] API returned ${searchData.files.length} total files. Filtering...`);
      const candidates = [];
      for (const file of searchData.files) {
        ...
        if (!/\.(mkv|mp4|avi)$/i.test(itemTitle)) {
          console.log(`[AFDS] Skipped (Not a video): ${itemTitle}`);
          continue;
        }
        const cleanTitle = normalize(itemTitle);
        const cleanQuery = normalize(titleQuery);
        if (!cleanTitle.includes(cleanQuery)) {
          console.log(`[AFDS] Skipped (Title Mismatch): ${itemTitle}`);
          continue;
        }
        if (year && !cleanTitle.includes(year)) {
          console.log(`[AFDS] Skipped (Missing Year ${year}): ${itemTitle}`);
          continue;
        }
        let quality = "Unknown";
        if (itemTitle.toLowerCase().includes("4k") || itemTitle.toLowerCase().includes("2160p"))
          ...
          quality = "720p";
        console.log(`[AFDS] >> KEEPING: ${itemTitle} (${quality})`);
        candidates.push({ id: dataId, category, title: itemTitle, size: itemSize, quality });
      }
      if (candidates.length === 0) {
        console.log("[AFDS] No candidates left after filtering.");
        return [];
      }
...
${file.quality} | \u{1F4E6} ${file.size}`,
      ...
      console.log(`[AFDS] Successfully resolved ${finalStreams.length} streams!`);
      return finalStreams;
    } catch (e) {
